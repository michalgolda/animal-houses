<template>
    <Box class="w-full">
        ...
        <div v-else class="mt-8 flex flex-col gap-y-4">
            <SearchResultItem @click="editProduct(result.item.id)" v-for="result in search.productResults.value"
    ...
    <Box class="w-full">
        ...
        <p v-if="search.attributeResults.value.length === 0" class="text-l mt-8 normalcase text-gray-400">Brak wyników
            wyszukiwania</p>
        <div v-else class="mt-8 flex flex-col gap-y-4">
            <SearchResultItem @click="editAttribute(result.item.id)" v-for="result in search.attributeResults.value"
                :id="result.item.id" :name="result.item.name" />
        </div>
    </Box>
</template>
